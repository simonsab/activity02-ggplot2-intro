...
**Response**: The fill feature sets a color for each species and
corresponds those colors to the legend on the right.
...
**Response**:The boxplot more clearly shows the median and quartiles of
the bill length. It’s easier to read at first glance whereas the
jittplot requires you focus more to understand what is being presented.
A pro of the jittplot could be that it gives an interesting view of the
distribution of bill length for each species because you see each
penguin in the data set represented as a dot, but it’s still not as
useful of a reporting visualation in this instance.
...
**Response**:It is far easier to read the .md file. It is what would
likely be used in a professional report, rather than the “raw” code of
the .Rmd file.
...
``` r
ggplot(data = penguins, mapping = aes(x = flipper_length_mm, y = species, fill = species)) + geom_jitter() + geom_boxplot()
```
...
**Response**:I prefer the boxplot with the jitterplot laid over. It
doesn’t make sense the other way because the boxes cover the majority of
the data points.
...
``` r
ggplot(data = penguins, mapping = aes(x = flipper_length_mm, y = species)) +
  geom_boxplot() + geom_jitter(aes(color = 'plum3'))
```
...
**Response**: There is a lot of flexibility with how you are able to
apply colors to variables within the charts.
